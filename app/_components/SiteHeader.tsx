    ...
    <header className="sticky top-0 z-50 border-b border-border bg-ink/85 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.08)] transition-all duration-300">
      <Container>
        <div className="flex h-16 sm:h-20 items-center justify-between gap-4">
          ...
          >
            <div className="relative flex h-10 w-[52px] sm:h-12 sm:w-[64px] items-center justify-center transition-transform duration-500 group-hover:scale-105 group-hover:drop-shadow-[0_0_15px_rgba(212,175,55,0.2)]">
              <LogoMark className="h-[135%] w-auto max-w-none" />
            </div>
            <div className="hidden leading-none sm:flex flex-col justify-center pt-0.5">
              ...
              <div className="mt-0.5 text-[9px] font-semibold uppercase tracking-[0.25em] text-gold/70 transition-colors duration-300 group-hover:text-gold">
                Surabaya
              </div>
            </div>
      ...
      >
        ...
        <div className="absolute inset-x-0 top-0 max-h-[calc(100svh-4rem)] overflow-y-auto bg-ink/95 backdrop-blur-xl border-b border-border px-5 py-8 pb-12 shadow-2xl">
          <nav className="flex flex-col gap-6" aria-label="Navigasi mobile">
            <Link
              href={homeItem.href}
              className={`text-xl font-medium tracking-tight transition-colors active:scale-95 ${isActive(homeItem.href) ? "text-gold" : "text-snow hover:text-gold"
                }`}
              onClick={() => setIsOpen(false)}
            >
              {homeItem.label}
            ...
            <div className="flex flex-col gap-4">
              <button
                type="button"
                className={`flex w-full items-center justify-between text-xl font-medium tracking-tight transition-colors active:scale-95 ${aboutActive ? "text-gold" : "text-snow hover:text-gold"
                  }`}
                ...
                {aboutItems.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.href}
                      href={item.href}
                      className={`text-lg font-medium tracking-tight transition-colors py-1 active:scale-95 ${active ? "text-gold" : "text-snow/80 hover:text-snow"
                        }`}
                      onClick={() => setIsOpen(false)}
                    >
                      {item.label}
            ...
            {secondaryItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`text-xl font-medium tracking-tight transition-colors active:scale-95 ${active ? "text-gold" : "text-snow hover:text-gold"
                    }`}
                  onClick={() => setIsOpen(false)}
                >
                  {item.label}
